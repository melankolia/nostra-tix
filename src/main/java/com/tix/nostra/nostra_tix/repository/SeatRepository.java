...
public interface SeatRepository extends JpaRepository<Seat, Long> {
    ...
    @Query("""
                SELECT s.id as id,
                    ...
                    s.isVisible as visible
                FROM Seat s
                WHERE s.studio.id = :studioId AND s.rowIndex = :rowIndex
                ORDER BY s.columnIndex ASC
            """)
    List<SeatByStudioIdProjection> findByStudioIdAndRowIndexProjectedBy(@Param("studioId") Long studioId,
